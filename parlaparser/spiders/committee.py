...
class CommitteeSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for party_link in response.css(".sub-navigation-list a::attr(href)").extract():
            yield scrapy.Request(
                url=self.base_url + party_link, callback=self.parse_orgs
            )
    ...
    def parse_orgs(self, response):
        ...
        for html_line in (
            response.css(".block_area_aside .content-toggle")[1].css("::text").extract()
        ):
            html_line = (
                html_line.replace(":", "").replace(",", "").replace("-", "").strip()
            )
            if html_line == "":
                continue
            if (
                html_line.strip().lower().startswith("predsedni")
                or html_line.strip().lower().startswith("podpredsedni")
                or html_line.strip().lower().startswith("člani")
                or html_line.strip().lower().startswith("strokovni sodelavec")
                or html_line.strip().lower().startswith("strokovna sodelavka")
            ):
                if role:
