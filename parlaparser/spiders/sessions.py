...
class SessionsSpider(scrapy.Spider):
    ...
    def __init__(
        self, parse_name=None, agenda_name=None, parse_type=None, *args, **kwargs
    ):
    ...
    def parse_voting_for_guest_free_and_presession_votes(
        self, response, basic_vote_data
    ):
        votes = {}
    ...
    def parse_session(self, response):
        ...
        parsed_votes, agenda_item = self.parse_attachment_vote(
            response,
            "Sprejeti dnevni red",
            -1,
            session_name,
        ...
        for li in response.css(".council-page--list-published-agendas>li"):
            ...
            if self.parse_type in ["votes", None]:
                ...
                is_added_agenda_item = self.update_votes_with_links(
                    votes, links, is_added_agenda_item
                )
    ...
    def find_links_and_enums(self, link, link_text_path="::text"):
        ...
        if range_enums:
            enums = self.words_orders[
                self.words_orders.index(range_enums[0][0]) : self.words_orders.index(
                    range_enums[0][1]
                )
                + 1
            ]
    ...
    def update_votes_with_links(self, votes, links, is_added_agenda_item=False):
        ...
        for key, vote in votes.items():
            ...
            votes[key].update({"links": tmp_links})
            # return vote
            is_added_agenda_item = True
